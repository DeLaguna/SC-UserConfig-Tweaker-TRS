...
function Implement-SystemTweaks {
    ...
    if ($decision -eq 0) {
        ...
        # Disable GameMode, GameBar, and Game DVR
...
function Get-SystemRAM {
    ...
    $totalMemoryMB = [math]::truncate($totalMemory / 1MB)
    ...
    $choiceIndex = $host.ui.PromptForChoice("System RAM", "We detected $totalMemoryMB MB of RAM. Select 'Auto Detected' to use the full amount or choose a different size (MB):", $choices, 0)
...
function Get-VideoCardMemory {
    ...
    $choiceIndex = $host.ui.PromptForChoice("Video Card VRAM", "Select 'Auto Detected' to use the recommended size or choose a different size (MB):", $choices, 0)

    # Return the user's choice or the auto-detected size
    $selectedVramSize = if ($choiceIndex -eq 0) {
        $autoOption
    } else {
        $vramOptions[$choiceIndex - 1]
    }
...
;System RAM Detected: $systemRAMChoice MB
;Video Card Memory Detected: $videoCardMemoryMB MB
...
sys_limit_phys_thread_count = $($systemInfo.PhysicalCores)
sys_job_system_enable = 1
sys_job_system_max_worker = $($systemInfo.PhysicalCores)
